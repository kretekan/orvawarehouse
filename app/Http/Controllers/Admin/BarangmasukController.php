...
class BarangmasukController extends Controller
{
    public function index()
    {
        ...
        // ✅ tambahan: ambil semua data barang
        $data["barang"] = BarangModel::orderBy('barang_id', 'DESC')->get();
                        ...
                        $button .= '
                            <div class="g-2">
                                <a class="btn modal-effect text-primary btn-sm" data-bs-effect="effect-super-scaled"
                                    ...
                                    data-bs-original-title="Edit" onclick=update(' . json_encode($array) . ')>
                                ...
                                <a class="btn modal-effect text-danger btn-sm" data-bs-effect="effect-super-scaled"
                                    data-bs-toggle="modal" href="#Hmodaldemo8" onclick=hapus(' . json_encode($array) . ')>
    ...
    public function proses_tambah(Request $request)
    {
        ...
        foreach ($request->barang as $key => $barangKode) {
            BarangmasukModel::create([
                'bm_tanggal' => $tgl,
                'bm_kode' => $kode,
                'barang_kode' => $barangKode,
                'customer_id' => $customer,
                'bm_jumlah' => $request->jml[$key],
            ]);
        }
